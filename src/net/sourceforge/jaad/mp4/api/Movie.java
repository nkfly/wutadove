...
public class Movie {
	...
	private CountDownLatch tracksFinishSignal;
	...
	class TrackWorker implements Runnable{
		...
		public TrackWorker(List<Box> trackBoxes,int startIndex, int endIndex){
		...
		public void run() {
			for (int i = startIndex;i < endIndex;i++){
				Track track = createTrack(trackBoxes.get(i));
				tracks.add(i, track);
			}
			tracksFinishSignal.countDown();
	...
	public Movie(Box moov, MP4InputStream in) {
		...
		tracksFinishSignal = new CountDownLatch(threadNumber);
		int partSize = trackBoxes.size()/threadNumber;
		for (int i = 0;i < threadNumber;i++){
			if (i == threadNumber-1){
				new Thread(new TrackWorker(trackBoxes,partSize*i, trackBoxes.size())).start();
				
			}else {
				new Thread(new TrackWorker(trackBoxes,partSize*i, partSize*(i+1))).start();
			}
		}
		
		try {
			tracksFinishSignal.await();
	...
	public List<Track> getTracks(Type type) {
		...
		for(Track t : tracks) {
			if(t != null && t.getType().equals(type)) l.add(t);
		}
	...
	public List<Track> getTracks(Track.Codec codec) {
		final List<Track> l = new ArrayList<Track>();
		for(Track t : tracks) {
			if(t != null && t.getCodec().equals(codec)) l.add(t);
		}
		return Collections.unmodifiableList(l);
	}
